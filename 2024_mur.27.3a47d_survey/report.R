...
if(save.plot)   taf.png(file.path(paste0('report/Figure.19.5.4.2.IBTSQ3_nums_1991_',wg_year-1,'selL')))
...
if(save.plot)   taf.png(file.path(paste0('report/Figure.19.5.4.1.BTSQ3_nums_1990_',wg_year-1,'selL')))
ggplot(data = spdf_NS) + geom_sf() + coord_sf(xlim = c(-5,15), ylim = c(49,62), expand = FALSE) + geom_point(data=dat, aes(x = lon, y = lat, shape = 4), size = 0.01, color = 'red') + scale_shape_identity() + geom_point(data=dat, aes(x = lon, y = lat, size = nums)) + facet_wrap(~Year, ncol = 8) + theme_bw()
if(save.plot)   dev.off()
...
if(save.plot)   taf.png(file.path(paste0('report/Figure.19.5.4.3.CGFS_nums_1989_',wg_year-1,'selL')))
...
# Figure 19.5.4.4 
taf.png(file.path('report/Figure.19.5.4.4.resid_year_tw'))
boxplot(res~Year, resid_tw, main = paste("Residual vs year"), las =2)
abline(h = 0, col = "red")
dev.off()

taf.png(file.path('report/Figure.19.5.4.4.resid_CShip_tw'))
boxplot(res~CShip, resid_tw, las =2, ann = F)
abline(h = 0, col = "red")
title(main = paste("Residual vs Ship-Country"))
dev.off()

taf.png(file.path('report/Figure.19.5.4.4.resid_Gear_tw'))
boxplot(res~Gear, resid_tw, main = paste("Residual vs Gear"), las =2)
abline(h = 0, col = "red")
dev.off()

taf.png(file.path('report/Figure.19.5.4.4.resid_timeofYear_tw'))
plot(res~timeOfYear, resid_tw, main = paste("Residual vs timeOfYear"))
abline(h = 0, col = "red")
dev.off()

taf.png(file.path('report/Figure.19.5.4.4.resid_TimeShotHour_tw'))
plot(res~TimeShotHour, resid_tw, main = paste("Residual vs TimeShotHour"))
abline(h = 0, col = "red")
dev.off()

taf.png(file.path('report/Figure.19.5.4.4.resid_Depth_tw'))
...
# Figure 19.5.4.5
taf.png("report/Figure.19.5.4.5.surveyQ34_index.png")
...
write.taf(table_summary, "report/Table.19.5.4.1.surveyQ34_index.csv")
...
# Figure 19.5.4.7 
taf.png("report/Figure.19.5.4.7.surveyQ34_index_tw_retro.png")
...
# Figure 19.5.4.6
taf.png("report/surveyQ34_index_tw_loo_std.png")
...
taf.png("report/Figure.19.5.4.6.surveyQ34_index_tw_loo_std_deviation.png")
...
taf.png("report/Figure.19.5.4.6.surveyQ34_index_tw_loo.png")
