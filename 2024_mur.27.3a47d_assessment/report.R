...
# Table 19.5.1.1. 
write.taf(table_summary[,c("Year", "Belgium", "Denmark", "France", "Germany", "Netherlands", "UK", "Total")] , file =paste0("report/Table.19.5.1.1.official_landings_country",assess_year,".csv"))
...
# Table 19.5.1.2. 
write.taf(table_summary , file =paste0("report/Table.19.5.1.2.official_landings_area",assess_year,".csv"))

#official landings
cbbPalette <- c("#000000", "#F0E442", "#009E73", "#CC79A7","#0072B2", "#E69F00")

#Figure 19.5.1.2.
taf.png("Figure.19.5.1.2.OfflandingArea.png")
...
ggplot(p, aes(x = as.numeric(Year), y = offLandings, group = Area, fill = Area)) + 
  ...
  scale_x_continuous(breaks=seq(1975, max(officialLandings$Year), by = 2)) + scale_fill_manual(values = cbbPalette) +
...
#Figure 19.5.1.1.
taf.png("Figure.19.5.1.1.OfflandingCountry.png")
...
# Table 19.5.1.3
output_summary <- read.taf("output/output_summary_ref2.csv")
...
write.taf(table_summary , file =paste0("report/Table.19.5.1.3.official_ices_landings_advice",assess_year,".csv"))

# Table in section 19.6.1 # when don't give advice section 19.6.3

table_summary <- output_summary %>% rename('Year'='year') %>% filter(Year > 2003)

# WARNING change section names when don't give advice
# write.taf(table_summary[, c('Year', 'Lc_year', 'Lc', 'Linf', 'LFeM', 'Lmean', 'indicator', 'index', 'landings', 'harvest_rate')] , file =paste0("report/Table.section.19.6.1.chr_rule",assess_year,".csv"))
write.taf(table_summary[, c('Year', 'Lc_year', 'Lc', 'Linf', 'LFeM', 'Lmean', 'indicator', 'index', 'landings', 'harvest_rate')] , file =paste0("report/Table.section.19.6.3.chr_rule",assess_year,".csv"))
...
ggplot(p, aes(x = as.numeric(Year), y = CatchWt, group = gear, fill = gear)) + 
  geom_bar(stat = "identity") + 
  scale_x_continuous(breaks=seq(2003, max(WtTot$Year), by = 2))  + scale_fill_manual(values = cbbPalette)+
  theme_bw() + theme(axis.text.x = element_text(angle = 45, vjust =0.5)) 
dev.off()

# Figure 19.5.1.5.
taf.png("Figure.19.5.1.5.IClandingGearperc.png")
...
# Figure 19.5.1.4.
taf.png("Figure.19.5.1.4.IClandingCountryperc.png")
...
taf.png("Figure.19.5.1.3.ICversusOffLandings.png")
...
write.taf(table_summary , file =paste0("report/Table.19.5.2.1.landing_num_age",assess_year,".csv"))

# Numbers at age (bubbles)
# Bubble plots
# Figure 19.5.2.2. 
taf.png(filename="Figure.19.5.2.2.Landingsbubble.png")
...
write.taf(table_summary , file =paste0("report/Table.19.5.2.2.landing_weight_age",assess_year,".csv"))
...
taf.png(filename="Figure.19.5.2.3.Landingswght.png")
ggplot(data = toto, aes(x=year,y=data, group = age, color = age)) + geom_line() + 
  geom_text(aes(label=ifelse(as.numeric(age)<4,age,paste0(age,"+")))) + ggtitle("Landings weight at age", )+ xlab("Year") + ylab("Weight (kg)") + 
...
# explore IC length sampling coverage in 2022
...
# Figure 19.5.3.4. 
load(file="data/IC_echantil_length_mur_wgnssk2024.rdata")
taf.png("Figure.19.5.3.4.LenghtdistributionYear.png")
p <- IC_echantil$Undetermined_length

p <- melt(data = p, id = "MeanLength", variable.name = "Year")
p$MeanLength <- as.numeric(p$MeanLength)
p <- p[p$value!=0,]
...
# taf.png("report/Figure.19.6.1.1.index_bpa_ref.png") # to change when RP's change
...
# Figure.19.6.1.2.
lc@value <- lc_ref@value # so we get 2023 length distribution with ref Lc maybe move it in output script
# taf.png("report/Figure.19.6.1.2.lfreq_lc.png") # so we get 2023
taf.png("report/Figure.19.6.3.2.lfreq_lc.png") # need to change when change RP's
cat3advice::plot(lc)
dev.off()
...
# taf.png("report/Figure.19.6.1.4.lfreq_lmean_ref2.png")
taf.png("report/Figure.19.6.3.3.lfreq_lmean_ref2.png") # need to change when change RP's
...
advice(fi_ref2)
taf.png("report/lfreq_lref_ref2.png")
cat3advice::plot(fi_ref2, y_label = "Length in mm")
dev.off()
...
# taf.png("report/Figure.19.6.1.5.FproxyMSY_ref2.png") # change when RP's change  
...
### ------------------------------------------------------------------------ ###
### Chr rule: cat3advice package from Simon Fisher compare with WGNSSK2023   ###  
### ------------------------------------------------------------------------ ###
# load assessment from last year
load(file = "boot/data/Chr_advice_2023.Rdata")
A_old <- A
advice_old <- advice
bi_old <- bi
fi_old <- fi
...
object2@idx$name = "Index-WGNSSK2023"
...
b_refs2 <- data.frame(name = c("I[loss]-WGNSSK2023", "I[trigger]-WGNSSK2023"),
...
p <- ggplot2::ggplot() +
  ...
  ggplot2::scale_linetype_manual("",
                                 values = c("Index-WGNSSK2023" = "solid",
                                            "Index" = "solid",
                                            "I[loss]" = "dotted", 
                                            "I[trigger]" = "solid",
                                            "I[loss]-WGNSSK2023" = "dotted",
                                            "I[trigger]-WGNSSK2023" = "solid"),
                                 labels = scales::parse_format()) + 
  ggplot2::scale_colour_manual("",
                               values = c("Index" = "#077c6c",
                                          "Index-WGNSSK2023" = "green",
                                          "I[loss]" = "blue",
                                          "I[trigger]" = "blue",
                                          "I[loss]-WGNSSK2023" = "#679dfe",
                                          "I[trigger]-WGNSSK2023" = "#679dfe"),
                                           labels = scales::parse_format()) +
  ggplot2::coord_cartesian(ylim = c(0, idx_max * 1.1),
                           xlim = c(yr_min - 1, yr_max + 1),
                           expand = FALSE) +
  ggplot2::labs(x = "", y = y_label,
                title = "Biomass Index comparisons WGNSSK2023/2024") +
...
taf.png("report/Figure.19.6.3.1.index_comparison_bpa_ref.png")
p
dev.off()
...
taf.png("report/Figure.19.6.3.4.FproxyMSY_ref2_comparison.png") # be careful if last year of data indicator above 1 use Fp_ref  
p
dev.off()