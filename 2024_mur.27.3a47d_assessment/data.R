...
taf.unzip("boot/data/OfficialNominalCatches.zip",files = "ICESCatchDataset2006-2021.csv", exdir = "boot/data")
taf.unzip("boot/data/2022preliminaryCatchStatisticsV2.zip", exdir = "boot/data")
taf.unzip("boot/data/2023preliminaryCatchStatistics.zip", exdir = "boot/data")
...
prelPrevYear <- grep("1pb203", official_files, value =T)
prelData <- grep("mzkaie", official_files, value =T)
...
HistoricalLandings <- HistoricalLandings %>% filter(Division %in% c("III a", "IV a", "IV b", "IV c", "IV (not specified)", "VII d") &
  ...
  melt(id.vars = c("Country", "Species", "Division"), variable.name = "Year", value.name = "AMS.Catch") %>%
...
officialLandings <- officialLandings %>% filter(Area %in% c("27.3.a", "27.4.a", "27.4.b", "27.4.c", "27.4_NK", "27.7.d") &
                                                  Species == "MUR") %>%
  melt(id.vars = c("Country", "Species", "Area", "Units"), variable.name = "Year", value.name = "AMS.Catch") %>%
  mutate(Year = as.numeric(gsub("X", "", Year)),
         Area = gsub("27.4_NK","27.4" , Area), Units = NULL, BMS.Catch = NA)


prelPrevYear <- read.csv(file.path("boot/data/", prelPrevYear))
...
prelData <- prelData %>% filter(Area %in% c("27_3_A_20", "27_3_A_21", "27_4_A", "27_4_B", "27_4_C", "27_7_D") &
                                     Species.Latin.Name == "Mullus surmuletus") %>%
  mutate(Area = gsubfn("27_3_A_20|27_3_A_21|27_4_A|27_4_B|27_4_C|27_7_D", 
                       list('27_3_A_20'="27.3.a",'27_3_A_21'="27.3.a",'27_4_A'="27.4.a",'27_4_B'="27.4.b", '27_4_C'="27.4.c", 
                            '27_7_D'="27.7.d"), Area),
...
# Explore raw IC
taf.unzip("boot/data/IC_age2003-2023.zip", exdir = "boot/data")
...
IC_landing_new <- data.frame(year = c(2024,2025), landings = NA, advice = 1985, discards = NA, catch = NA)
...
Landing_sum <- rbind(Landing_sum, officialLandings) %>% filter(Year > 2003)
...
#### explore Length data 
taf.unzip("boot/data/IC_length2022-2023.zip", exdir = "boot/data")
...
load(file="boot/data/IC_echantil_length_mur_wgnssk2023.rdata")

## read new data 2022
N2022 <- readVPAIntercatch_length('boot/data/mur.27.3a47d_all_ 2024-4-5 14_33_10_2022L_Length/canum.txt')
W2022 <- readVPAIntercatch_length('boot/data/mur.27.3a47d_all_ 2024-4-5 14_33_10_2022L_Length/weca.txt')
...
IC_echantil$Undetermined_length <- IC_echantil$Undetermined_length[,-dim(IC_echantil$Undetermined_length)[2]] #remove old 2022 data
...
IC_echantil$Undetermined_weight <- IC_echantil$Undetermined_weight[,-dim(IC_echantil$Undetermined_weight)[2]] #remove old 2022 data
...
## read new data 2023
N2023 <- readVPAIntercatch_length('boot/data/mur.27.3a47d_all_ 2024-4-5 15_24_44_2023L_Length/canum.txt')
W2023 <- readVPAIntercatch_length('boot/data/mur.27.3a47d_all_ 2024-4-5 15_24_44_2023L_Length/weca.txt')
...
save(IC_echantil, file="data/IC_echantil_length_mur_wgnssk2024.rdata")

# relocate survey index Q34 file
write.taf(read.taf("boot/data/GAM_MURQ34_indices1991-2023_tw.csv"), file = "data/GAM_MURQ34_indices1991-2023_tw.csv") 
