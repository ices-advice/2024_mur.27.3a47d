...
length_mur.27.3a47d <- length_mur.27.3a47d %>% melt(id.vars = "MeanLength", variable.name = "Year", value.name = "numbers") %>%
...
lc <- cat3advice::Lc(length_mur.27.3a47d, average = T) # with last year of data
...
lmean <- cat3advice::Lmean(data = length_mur.27.3a47d, Lc = lc, units = "mm")
...
lref <- cat3advice::Lref(Lc = lc, Linf = 341, units = "mm")
...
fi <- cat3advice::f(Lmean = lmean, Lref = lref, units = "mm")
...
advice <- cat3advice::chr(A = A, I = idx, F = Fp, b = bi, m = mp, discard_rate = 0)
...
save(Ind,A,idx,lc,lc_ref,lmean,lmean_ref,lmean_ref2,lref, lref_ref, fi,fi_ref,fi_ref2,hr,hr_ref,Fp,Fp_ref,Fp_ref2,bi,bi_ref,mp,advice,advice_ref,advice_ref2, file = "model/Chr_advice_2024.Rdata")
